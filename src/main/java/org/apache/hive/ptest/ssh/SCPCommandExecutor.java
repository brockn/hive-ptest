...
public class SCPCommandExecutor {
  ...
  public void execute(SCPCommand command) {
    CollectPolicy collector = new CollectPolicy();
    try {
      LocalCommand cmd;
      if(command.getType() == SCPCommand.Type.TO_LOCAL) {
        cmd = new LocalCommand(collector,
            String.format("scp -B -C -v -i %s -r %s@%s:%s %s", command.getPrivateKey(), 
                command.getUser(), command.getHost(), command.getRemoteFile(),
                command.getLocalFile()));
      } else if(command.getType() == SCPCommand.Type.FROM_LOCAL) {
        cmd = new LocalCommand(collector,
            String.format("scp -B -C -v -i %s -r %s %s@%s:%s", command.getPrivateKey(), 
                command.getLocalFile(), command.getUser(), command.getHost(),
